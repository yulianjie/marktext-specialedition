<template>
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      <div class="text">
        {{ $t('keybindings.instructions') }}
        <a class="link" @click="openKeybindingWiki">{{ $t('keybindings.online') }}</a>.
      </div>
      <el-table
        :data="keybindingList"
        style="width: 100%"
      >
        <el-table-column prop="description" :label="$t('keybindings.columns.description')">
        </el-table-column>
        <el-table-column prop="accelerator" :label="$t('keybindings.columns.key')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('keybindings.columns.options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('keybindings.row.edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('keybindings.row.reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('keybindings.row.unbind')">
    ...
    <section class="footer">
      <separator></separator>
      <el-button size="medium" @click="saveKeybindings">{{ $t('keybindings.buttons.save') }}</el-button>
      <el-button size="medium" @click="restoreDefaults">{{ $t('keybindings.buttons.restoreDefaults') }}</el-button>
    </section>
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{ $t('keybindings.debugHeading') }}</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{ $t('keybindings.buttons.debug') }}</el-button>
    </section>
    <key-input-dialog
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('keybindings.notify.saveFailedTitle'),
                type: 'error',
                message: this.$t('keybindings.notify.saveError')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('keybindings.notify.inUseTitle'),
        type: 'warning',
        message: this.$t('keybindings.notify.inUseMessage', { accelerator })
      })
    },
